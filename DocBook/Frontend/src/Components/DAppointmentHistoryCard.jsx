...
const DAppointmentHistoryCard = ({ appointment, onDeleteAppointment }) => {
  const {
    fee,
    noOfPatient,
    time,
  ...
  return (
    <div className="w-64 bg-white border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow">
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-gray-700 mb-3">Time: {time}</p>

        <button
          onClick={() => onDeleteAppointment(appointment.id)}
          
          className="mt-3 bg-red-500 text-white px-3 py-1.5 rounded text-sm hover:bg-red-700 w-full transition-colors"
        >
          Delete Appointment
        </button>
      </div>
...
export default DAppointmentHistoryCard;
